...
export const CommentEditorWrapper: React.FC<Props> = ({
  ...
  const model = genAI.getGenerativeModel({
    ...
    systemInstruction:
      "现在你是DeepCosmo平台的智能 AI系统。并且你能获得当前页面的基本的页面数据：" +
      JSON.stringify(state.pageInfo) +
      "。同时你将获得当前页面的相关调用函数，并且你需要根据这些函数的调用情况来做出自动化的调用。请全程使用中文沟通，保存你的智能，切记你是DeepCosmo平台的智能 AI助手",
    tools: [tools],
  });
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      if (Array.isArray(functionCalls) && functionCalls.length > 0) {
        ...
        const autoResponses = [] as { name: string; response: any }[];
