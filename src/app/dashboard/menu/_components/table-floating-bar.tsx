...
import { deleteSystemMenus, updateSystemMenus } from "../_lib/actions";
...
export function TableFloatingBar({ table }: SystemMenuTableFloatingBarProps) {
	...
	const [action, setAction] = React.useState<
		"update-menuType" | "update-priority" | "export" | "delete"
	>();
				...
				<div className="w-full overflow-x-auto">
					<div className="mx-auto flex w-fit items-center gap-2 rounded-md border bg-background p-2 text-foreground shadow">
						...
						<div className="flex items-center gap-1.5">
						  <Select
							  onValueChange={(value: SystemMenu["menuType"]) => {
								setAction("update-menuType")

								startTransition(async () => {
									const { error } = await updateSystemMenus({
							...
							>
									...
									<SelectTrigger asChild>
										<TooltipTrigger asChild>
											<Button
												...
												disabled={isPending}
											>
												{isPending && action === "update-menuType" ? (
													<Loader
								...
								<SelectContent align="center">
									<SelectGroup>
										{systemMenu.menuType.enumValues.map((type) => (
											<SelectItem
												key={type}
												value={type}
												className="capitalize"
											>
												{getMenuTypeContent(type).text}
											</SelectItem>
										))}
									</SelectGroup>
								</SelectContent>
